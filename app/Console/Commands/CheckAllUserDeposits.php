...
class CheckAllUserDeposits extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:check-all-user-deposits';
     ...
     * @var string
     */
    protected $description = 'Checks all user deposits';
    ...
    public function handle()
    {
        ...
        $semaphore = 0;
        if ($is_linux) {
            $semaphore = sem_get(2021022210, 1);
            if (empty($semaphore)) {
        ...
        print_r("!!!!!!!!!!!!!!!!!! --- Check All User Deposit --- !!!!!!!!!!!!!!!!!!" . "\n");
        Log::channel('crypto')->info("!!!!!!!!!!!!!!!!!! --- Check All User Deposit --- !!!!!!!!!!!!!!!!!!");
        ...
        $userLists = Trader::getAllUserIds();
		$total_count = 0;
        foreach ($userLists as $user_index => $user_record) {
            $req_id = $user_record->id;
            ...
            foreach ($walletLists as $record) {
                ...
                print_r('>> Checking wallets(' . $req_id . ') : ' . $currency . ', ' . $wallet_address . "\n");
                Log::channel('crypto')->info("");
                Log::channel('crypto')->info('>> Checking wallets(' . $req_id . ', ' . $total_count . ') : ' . $currency . ', ' . $wallet_address);
                ...
                if ($balanceInfo === false) {
					...
					Log::channel('crypto')->info("!!!!! Check balance has failed!");
                    continue;
                }
                ...
                if ($currency != 'USDT' && $currency != 'ETH') {
                    ...
                    if ($gasPriceMode != GASPRICE_MANUAL) {
                        if ($currency == 'BTC') {
                            $fee_unit = g_getBTCFees()[$gasPriceMode];
                        }
                    }
                }
    ...
    public function checkWalletDeposit($currency, $addr)
    {
        ...
        $balance = Decimal::create($balanceInfo['detail'] == '' ? 0 : $balanceInfo['detail']);
