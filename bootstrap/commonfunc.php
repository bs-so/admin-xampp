...
function g_getBTCFees() {
    $ret = g_sendHttpRequest(BTC_FEE_URL, HTTP_METHOD_GET);
    $ret = json_decode($ret, false);

    return array(
        GASPRICE_FAST       => intval($ret->fastestFee / 3),
        GASPRICE_STANDARD   => intval($ret->halfHourFee / 3),
        GASPRICE_SAFELOW    => intval($ret->hourFee / 3),
    );
}

function g_getGasPrices() {
    $ret = g_sendHttpRequest(ETH_FEE_URL, HTTP_METHOD_GET);
    $ret = json_decode($ret, false);

    return array(
        GASPRICE_FAST       => $ret->fastest,
        GASPRICE_STANDARD   => $ret->fast,
        GASPRICE_SAFELOW    => $ret->safeLow,
    );
}

function g_getBTCTxSizeInBytes($in, $out) {
    // in*180 + out*34 + 10 plus or minus 'in'
    return $in * 180 + $out * 34 + 10 + $in;
}
